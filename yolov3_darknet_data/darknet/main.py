#https://github.com/BASFProject-2018spring/BBox-Label-Tool-large-img
#-------------------------------------------------------------------------------
...
##        self.loadDir()
    ...
    def mouseClick(self, event):
        if self.STATE['click'] == 0:
            ...
            bboxList3 = (x2-x1)/self.tkimg.width()
            bboxList4 = (y2-y1)/self.tkimg.height()
            bboxList1 = (x1+bboxList3*self.tkimg.width()/2)/self.tkimg.width() 
            bboxList2 = (y1+bboxList4*self.tkimg.height()/2)/self.tkimg.height() 
            self.bboxList.append((self.currentLabelclass, float(bboxList1), float(bboxList2), float(bboxList3), float(bboxList4)))
